...
def exporta_color():
    ...
    for index in range(len(df_anual)):
        #if index > 0:
            total.append((  (df_anual['litros'].loc[index-1] / totlitros ) *100 ))
            tot1.append((  (df_anual['fob'].loc[index-1] / totfob *100 )))
            tot2.append((  (df_anual['fob'].loc[index-1] / df_anual['litros'].loc[index]) )    )
        #st.write(total)
